...
function [MSEbatch, PSNRbatch, SSIMbatch, diffErr, SSIM_filtered] = analyzeSelectiveBatchGroups(V, Kernels, videofilename, groupNames)
...
% Prepare saving of masked filtered movies for any group passed in
saveDir = 'MoviesProcessed_ON_[83,83]/';
if ~exist(saveDir, 'dir')
    mkdir(saveDir)
end
fname = [extractBefore(videofilename, '.mp4') '_'];
% Allow for user to pass in only sustained or only transient
...
% Add MSE between Partial and DTR (Partial-DTR)
% Note 20240515 that length(groupNames) was changed to length(diffErr)
% to account for cases where sustained is passed, but not transient, or
% vice versa
if ~isempty(sustained)
    diffErr(length(diffErr)+1,:) = mean((Vfilts.(suPartialNm) - Vfilts.(suDTRNm)).^2, [1,2]); % Adding sustained Partial-DTR (MSE at 7th row)
elseif ~isempty(transient)
    diffErr(length(diffErr)+1,:) = mean((Vfilts.(trPartialNm) - Vfilts.(trDTRNm)).^2, [1,2]); % Adding transient Partial-DTR (MSE at 8th row)
end
