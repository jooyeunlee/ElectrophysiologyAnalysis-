...
function doLNmembrane()
	
	wave allResp
	variable howMany=dimsize(allResp,1)
	...
	for(i=0;i<howMany;i+=1)
		...
		lines[][0][]=stim0[p][r]
		killwaves /z stim0
		wave thisMem=$"wout"+num2str(i)
		make /o/n=(numpnts(thisMem)) linesRF_v
		setscale /p x,leftx(thisMem),deltax(thisMem),linesRF_v
		linesRF_v=thisMem[p]
		if(deltax(linesRF_v)<0.001)
			reduceSamplingInPlace("linesRF_v",0.001)
		endif
	...
	doNL(lp,v,50)
	wave NLy,NLx,NLySD
	duplicate /o NLy memNLy
	duplicate /o NLx memNLx
	duplicate /o NLySD memNLySD
	duplicate /o lp memlp
	
	RF/=dimdelta(RF,0)
	duplicate /o RF memRF
	
	killWindow /z linesGraphMem
	doLinesGraphMem()
...
function doLinesGraphV()
	...
	ModifyGraph nticks(l1)=3
End

function doLinesGraphSp()
	wave spRF,spNLy,spNLx,spNLysd
	Display /k=1/n=linesGraphSp/W=(12,296,261,391)/L=l1/B=b1 spNLy vs spNLx
	AppendImage spRF
	...
	Label left "\\Z12\\F'Helvetica'Size (mm)"
	Label bottom "\\Z12\\F'Helvetica'Delay (s)"
	Label l1 "\\Z12\\F'Helvetica'Output (Hz)"
	Label b1 "\\Z12\\F'Helvetica'Input"
	ErrorBars/T=0/L=2 spNLy Y,wave=(spNLySd,spNLySd)
	ModifyGraph nticks(l1)=3
End

function doLinesGraphMem()
	wave memRF,memNLy,memNLx,memNLysd
	Display /k=0/n=linesGraphMem/W=(12,417,259,512)/L=l1/B=b1 memNLy vs memNLx
	AppendImage memRF
	ModifyImage memRF ctab= {-5,5,RedWhiteBlue,0}
	ModifyGraph mode=3
	ModifyGraph marker=19
	ModifyGraph rgb=(0,0,0)
	ModifyGraph mirror(left)=0,mirror(bottom)=0
	...
	Label l1 "\\Z12\\F'Helvetica'Output (mV)"
	Label b1 "\\Z12\\F'Helvetica'Input"
	ErrorBars/T=0/L=2 memNLy Y,wave=(memNLySd,memNLySd)
	ModifyGraph nticks(l1)=3
End

function doLinesGraphSpRF()
	wave spRF
	Display /k=1/n=GraphSp/W=(364,512,701,768)
	AppendImage spRF
	ModifyImage spRF ctab= {-5,5,RedWhiteBlue,0}
	ModifyGraph mode=3
	ModifyGraph marker=19
	ModifyGraph rgb=(0,0,0)
	ModifyGraph mirror(left)=0,mirror(bottom)=0
	ModifyGraph nticks(left)=2,nticks(bottom)=2
	ModifyGraph fSize=10
	ModifyGraph standoff=0
	ModifyGraph nticks(left)=3
End

function doLinesGraphmemRF()
	wave memRF
	Display /k=1/n=Graphmem/W=(364,512,701,768)
	AppendImage memRF
	ModifyImage memRF ctab= {-5,5,RedWhiteBlue,0}
	ModifyGraph mode=3
	ModifyGraph marker=19
...
function doLinesGraphSpLN()
	wave spNLy,spNLx,spNLysd
	Display /k=1/n=linesGraphSpLN/W=(364,512,701,768)/L=l1/B=b1 spNLy vs spNLx
	ModifyGraph mode=3
	ModifyGraph marker=19
	ModifyGraph nticks(l1)=3,nticks(b1)=3
	ModifyGraph fSize=18,font="Helvetica"
	ModifyGraph standoff=0
	ModifyGraph lblPosMode(l1)=3,lblPosMode(b1)=1
	ModifyGraph lblPos(l1)=28
	ModifyGraph ZisZ=1
	ModifyGraph zapTZ=1
	ModifyGraph btLen=1.5
	ModifyGraph freePos(l1)={0.05,kwFraction}
	ModifyGraph freePos(b1)={0,kwFraction}
	ModifyGraph axisEnab(b1)={0.05,1}
	ErrorBars/T=0/L=2 spNLy Y,wave=(spNLySd,spNLySd)
	ModifyGraph nticks(l1)=3
End

function doLinesGraphmemLN()
	wave memNLy,memNLx,memNLysd
	Display /k=1/n=GraphmemLN/W=(364,512,701,768)/L=l1/B=b1 memNLy vs memNLx
	ModifyGraph mode=3
	ModifyGraph marker=19
	ModifyGraph nticks(l1)=3,nticks(b1)=3
	ModifyGraph fSize=18,font="Helvetica"
